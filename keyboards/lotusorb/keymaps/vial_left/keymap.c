...
static void apply_scroll_from_left(report_mouse_t *out, report_mouse_t left_report) {
    ...
    // 右トラボのSCROLLINGと同じ「steps」を使ったロジック
    int8_t steps_v = 0;
    int8_t steps_h = 0;
    ...
    // ★ 左トラボだけ倍率を掛けてスクロール量を大きくする
    int16_t delta_h = steps_h * LEFT_SCROLL_H_MULT * (user_config.mouse_scroll_h_reverse ? -1 : 1);
    int16_t delta_v = -steps_v * LEFT_SCROLL_V_MULT * (user_config.mouse_scroll_v_reverse ? -1 : 1);
